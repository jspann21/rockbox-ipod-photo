...
time at which the first completed strip reached the panel. On a build without a
microsecond timer, a DC-only first strip that completes within one scheduler
tick can be reported as `< timer resolution` rather than as a positive number.
