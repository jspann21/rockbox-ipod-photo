...
def main() -> None:
    ...
    ap.add_argument(
        "--validation-report",
        type=Path,
        help="optionally write the independent validation result as JSON",
    )
    ns = ap.parse_args()
    try:
    ...
    report_path = ns.validation_report
    if report_path is not None:
        ...
        if (ns.output.resolve() in report_targets or
            ...
            ap.error(
                "validation report and temporary report must not replace "
                "source or output"
            )
        # A failed or interrupted run must not retain an earlier PASS report.
    ...
    if report_path is not None:
        ...
        try:
            ...
            raise
        print(f"independent validation: PASS; report={report_path}")
    else:
