 ...
 *
 * Video-only AVI/MJPG qualification player. Frames are decoded with the
 * baseline JPEG decoder used by Image Viewer and rendered as RGB565.
 ****************************************************************************/
...
#define MJPEG_TRACE_PATH ROCKBOX_DIR "/mjpeg15.trace"
...
/* Rockbox's own JPEG viewer deliberately keeps this object off-stack. */
static struct jpeg frame_jpg;
...
static void trace_stage(unsigned int stage, unsigned int frame)
{
    const char *name;
    int fd;
    ...
    fd = rb->open(MJPEG_TRACE_PATH, O_WRONLY | O_CREAT | O_APPEND, 0666);
    if (fd < 0)
        return;

    name = rb->strrchr(trace_filename, '/');
    name = name != NULL ? name + 1 : trace_filename;
    rb->fdprintf(fd, "%s,%u,%u,%lu\n", name, stage, frame,
                            ...
                            struct avi_info *avi, int depth)
{
    ...
    while (pos + 8 <= end)
    {
        ...
        size = le32(header + 4);
        data = pos + 8;
        if (data > end || (uint64_t)size > (uint64_t)(end - data))
            return false;
...
{
    bool d0 = (id[0] >= '0' && id[0] <= '9') ||
              (id[0] >= 'A' && id[0] <= 'F');
    bool d1 = (id[1] >= '0' && id[1] <= '9') ||
              (id[1] >= 'A' && id[1] <= 'F');
                           ...
                           off_t *frame_offset, uint32_t *frame_size)
{
    while (true)
    {
        ...
        chunk_end = data + size;
        next = chunk_end + (size & 1u);
        if (next < chunk_end || next > cursor->end + 1)
            return false;
        ...
        if (id == AVI_LIST)
        {
            ...
            if (size < 4 || !read_at(fd, data, list_type, 4))
                return false;
            type = le32(list_type);
...
static bool alloc_frame_buffers(unsigned char *buffer, size_t buffer_size,
                                ...
                                unsigned char *planes[3], fb_data **rgb,
                                int *width, int *height, int *stride)
{
    ...
    *stride = frame_jpg.x_phys / ds;
    *width = frame_jpg.x_size / ds;
    *height = frame_jpg.y_size / ds;
    if (*stride <= 0 || *width <= 0 || *height <= 0 || cursor > end)
        return false;

    if (frame_jpg.blocks > 1)
    {
        for (i = 1; i < 3; i++)
        {
            int sx = frame_jpg.subsample_x[i];
            int sy = frame_jpg.subsample_y[i];

            if (sx <= 0 || sy <= 0)
                return false;
            size = (size_t)(frame_jpg.x_phys / ds / sx) *
                   (frame_jpg.y_phys / ds / sy);
    ...
    size = (size_t)(*stride) * (frame_jpg.y_phys / ds);
    if (size > (size_t)(end - cursor))
        return false;
    planes[0] = cursor;
    cursor += size;
...
static bool decode_frame(unsigned char *buffer, size_t buffer_size,
                         uint32_t compressed_size, bool first_frame,
                         ...
                         uint32_t *decode_us)
{
    ...
    rb->memset(&frame_jpg, 0, sizeof(frame_jpg));
    started = now_us();

    status = process_markers(buffer, compressed_size, &frame_jpg);
    if (first_frame)
        trace_stage(6, 0);
    if (status < 0 || (status & (DQT | SOF0)) != (DQT | SOF0) ||
        frame_jpg.table_error != 0)
        return false;

    if (!(status & DHT))
        default_huff_tbl(&frame_jpg);
    build_lut(&frame_jpg);

    ds = choose_ds(&frame_jpg);
    if (ds == 0 || !alloc_frame_buffers(buffer, buffer_size,
                                        compressed_size, ds, planes, &rgb,
                                        &width, &height, &stride))
        return false;

    jpeg_decode_set_mcu_row_callback(NULL, NULL);
    jpeg_decode_set_mcu_row_reuse(false);
    if (jpeg_decode(&frame_jpg, planes, ds, NULL) != 0)
        return false;
    if (first_frame)
        trace_stage(7, 0);
    ...
    if (!yuv_bitmap_part_to_buffer(
            planes,
            frame_jpg.blocks > 1 ? frame_jpg.subsample_x[1] : 0,
            frame_jpg.blocks > 1 ? frame_jpg.subsample_y[1] : 0,
...
static int poll_button(bool *paused)
{
...
static enum plugin_status play_avi(const char *filename)
{
    ...
    trace_enabled = path_exists(MJPEG_LOG_ENABLE);
    trace_stage(1, 0);
    ...
    if (buffer == NULL || buffer_size < 192 * 1024)
    {
        rb->close(fd);
    ...
    while (true)
    {
        ...
        rb->lcd_bitmap_part(rgb, 0, 0, width, x, y, width, height);
    ...
    rb->close(fd);
    trace_stage(9, stats.frames);
    log_stats(filename, &avi, &stats);

    return stats.frames != 0 ? PLUGIN_OK : PLUGIN_ERROR;
}
