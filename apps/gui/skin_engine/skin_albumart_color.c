...
/* Themify's vector widgets use a low-opacity inverse of the canvas for their
 * quiet surfaces.  Preserve that role without baking one album's beige into
 * the skin: 0x292d3a is the original dark-theme widget colour and acts as the
 * semantic marker for this adaptive surface. */
static unsigned int muted_surface(unsigned int dominant)
{
    int r = RGB_UNPACK_RED(dominant);
    int g = RGB_UNPACK_GREEN(dominant);
    int b = RGB_UNPACK_BLUE(dominant);

    return LCD_RGBPACK(r + (((255 - r - r) * 36) >> 8),
                       g + (((255 - g - g) * 36) >> 8),
                       b + (((255 - b - b) * 36) >> 8));
}
...
static void extract_colors(const struct bitmap *bmp)
{
    ...
    {
        unsigned int px = (unsigned int)pixels[i];
        int r4 = RGB_UNPACK_RED(px) >> 4;
        int g4 = RGB_UNPACK_GREEN(px) >> 4;
        int b4 = RGB_UNPACK_BLUE(px) >> 4;
    ...
    for (i = 0; i < HISTOGRAM_BUCKETS; i++)
    {
        ...
        /* Downscaling 160px Rockpod art to the Photo's 110px slot can split
         * one shadow across adjacent 4-bit buckets. Avoid an unstable
         * near-black winner so the same artwork keeps the same accent. */
        if (lum < 30)
            continue;
...
{
    if (original == LCD_RGBPACK(0x29, 0x2d, 0x3a))
        return muted_surface(eff_dominant);
...
{
    /* "muted" is a semantic skin colour, represented internally by the
     * legacy Themify marker.  It must follow the configured background even
     * when album-art colours are disabled or have not been extracted yet. */
    if (original == LCD_RGBPACK(0x29, 0x2d, 0x3a) &&
        (!global_settings.dynamic_colors || !cache.valid))
        return muted_surface(cache.theme_bg);
