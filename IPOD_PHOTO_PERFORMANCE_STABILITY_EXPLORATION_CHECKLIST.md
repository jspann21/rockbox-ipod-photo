...
- [x] LCD C polling and both assembly YUV FIFO loops use a shared elapsed-time/deadline contract. Assembly FIFO aborts return the failed FIFO phase, stop the rectangle without advancing source pointers, clear `LCD2_BLOCK_CONFIG`, latch recovery, and force a canonical framebuffer redraw on the next display call.
...
- [x] Intent-aware adaptive buffering is implemented in `17d34c0b42`: current audio receives deadline treatment; sequential iPod Color reads grow conservatively from 32 to 64 KiB only while the queue stays idle; interactive images use 64 KiB; metadata remains at 32 KiB; other targets retain 32 KiB behavior. A1099 qualification passed normal audio and IPVF playback without observed stutter or audio gaps. The unused 128 KiB policy remains disabled pending a separate measured need.
- [x] The installed LCD/PhotoDB batch uses 16-byte-aligned PhotoDB frame buffers so 77,440-byte F1013 reads qualify for PP5020 DMA, rejects negative file sizes and signed `off_t` overflow, validates YUV geometry before 4:2:0 rounding, and records LCD timing, requested/transmitted pixels, timeout phase, abandoned rectangles, recovery count, and failure streak in RAM. The A1099 smoke run transferred 468,107,776 bytes in 64,365 DMA requests with zero DMA/LCD/FIFO timeouts, PIO recoveries, abandoned rectangles, display recoveries, PCM underruns, resets, corruption, or PhotoDB cache errors.
- [x] PhotoDB startup scanning reuses the already-allocated aligned screen-frame buffer as a 64 KiB scan window instead of a separate 4 KiB buffer, parses complete `mhni` records directly from that window, and uses one bounded aligned fallback read only when a record crosses the window boundary. This reduces scan requests and seek/reread churn without increasing the plugin's steady-state memory requirement. The launcher now uses the iPod Photo's historical `/Photos/Photo Database` layout while retaining `/iPod_Control/Photos/Photo Database` as a compatibility fallback. The decoder accepts both the documented F1013 cache and the device's legacy F1020 cache, locks each database scan to one screen format to prevent duplicates, validates the 77,440-byte frame geometry, and opens the matching `Fxxxx_N.ithmb` file. The device's copied database contains 145 valid F1020 frames; host-side decoding confirmed its big-endian RGB565 byte order and existing 176x220 rotation against the matching original photo. `photodb.ovl` is categorized and packaged under `rocks/viewers`, matching the runtime loader path. The corrected compatibility build passed on-device launch and navigation testing: the 145-photo library opened quickly, displayed correctly, and produced no observed errors. The screen-sized PhotoDB viewer intentionally has no zoom mode.
...
- [x] **P0:** add counters for LCD busy-wait timeout, block-ready timeout, TXOK timeout, abandoned rectangle, controller reinitialization, and consecutive failure streak.
- [x] **P0:** change `lcd_write_yuv420_lines()` to return status (or set a driver-owned failure latch). On FIFO abort, stop the current rectangle immediately, clear `LCD2_BLOCK_CONFIG`, record whether FIFO 1 or FIFO 2 failed, and schedule a controlled reinitialization/full redraw instead of advancing as if two lines were transmitted.
- [x] **P0:** replace CPU-frequency-dependent iteration limits in both C and assembly LCD polling with one elapsed-time/deadline contract; preserve a tiny bounded fast poll only if its measured cost is lower than a timer check.
...
- [x] **P0:** validate and clip YUV source/destination geometry *before* rounding width/height to 4:2:0 alignment; reject negative coordinates, impossible extents, overflow, and any even-rounding that would cross the LCD or source plane. The API does not carry source-plane height, so callers still own that precondition; the driver validates all geometry and stride information it receives.
...
- [x] **P1:** instrument requested versus transmitted rectangle area to find themes/screens that invalidate the whole 220x176 framebuffer unnecessarily.
...
- [ ] **P0:** repeat the 256 MiB round-trip hash test after each ATA, cache, filesystem, USB, clock, or power change; add a multi-gigabyte pass before release. The installed `95e786aa0fM-260902` ATA1 qualification build passed a 256 MiB Windows-host-to-iPod-to-host round trip: source, on-device, and readback SHA-256 were all `a6d72ac7690f53be6ae46ba88506bd97302a093f7108472bd9efc3cefda06484`. The PIO host-to-iPod write measured 8.44 MiB/s; the immediate read was served from Windows cache and is deliberately not reported as device throughput. Subsequent normal-use playback/storage-wake testing completed with no issue reported, and a performance dump was captured. The multi-gigabyte release pass remains pending.
...
- [ ] **P0:** verify LBA28/LBA48 boundary arithmetic, logical sector size, capacity overflow, partition bounds, and end-of-device reads/writes for the installed capacity. The driver now splits every request into commands of at most 256 sectors under one absolute request deadline, preventing the LBA28 eight-bit sector count from wrapping while DMA treats a larger buffer as complete. Every successful post-reset and rail-wake `IDENTIFY` is reparsed into one geometry snapshot; capacity, LBA mode, logical-sector size, and multiple-sector count must match the accepted boot profile or recovery fails instead of issuing commands with stale geometry. Installed boundary and large-transfer qualification remains pending.
- [x] **P0 — implementation complete:** select a power-of-two `SET MULTIPLE MODE` count no larger than IDENTIFY word 47's advertised maximum (including non-power-of-two maxima such as 24); use valid word 59 only when word 47 is unavailable and fall back to one sector when neither supplies a count. This prevents a raw bridge value or contradictory current-count field from defining an unsafe PIO DRQ block size. The installed iFlash profile still selects one sector per DRQ. WSL build, device boot and normal USB transfers passed; malformed IDENTIFY profiles and recovery faults remain code-reviewed, not hardware-injected.
...
- [x] **P0:** write DMA selected only by `ata_disk_isssd()` is treated as unqualified on iPod Color. The target now uses PIO writes as its production safety baseline while retaining aligned UDMA1 reads; any future write-DMA allowlist must be narrow, versioned, opt-in first, and fall back conservatively when IDENTIFY data changes. On-device Debug was reported as `DMA configured/current: UDMA1/UDMA`, `DMA reads: >=4096 B; writes: PIO`, zero DMA timeouts, and zero successful/failed PIO recoveries before the successful 256 MiB integrity pass.
...
- [x] **P1:** Debug and an explicitly saved performance snapshot record configured mode, post-SET-FEATURES IDENTIFY-current mode, the 4096-byte read-DMA crossover, and whether write DMA is enabled. Never infer active UDMA1/2 from a compile-time cap or one diagnostic field.
- [ ] **P1:** qualify any write-DMA policy with matched controls, low-voltage interruption, reset during transfer, repeated hashes, and power-cycle tests; ship it only after a demonstrated benefit without recovery or integrity regression. An experimental exact allowlist for the installed `iFlash-Platform iPod Adapters` / `iFlash85` / 125,042,688-sector profile enabled UDMA1 writes and completed three 256 MiB transfers with matching SHA-256 plus zero DMA timeouts and zero PIO recoveries. It averaged 7.16 MiB/s (7.12, 7.20, 7.15), below the historical 8.44 MiB/s PIO result, so the experiment was removed and production PIO writes restored. Later PIO controls averaged 5.37 MiB/s, demonstrating session variability: the historical comparison does not establish that DMA itself was slower. A write-DMA benefit remains unproven; interruption/power qualification was not performed.
- [x] **P1:** one five-second absolute read/write deadline now begins before storage wake and is shared by rail-on/hard-reset waits, DMA completion, soft reset and reinitialization, and the single PIO rescue. The PP5020 DMA wait clamps itself to the caller's deadline, reset retains its longer ceiling only when invoked outside a request, and recovery no longer grants itself a fresh transfer budget. The installed A1099 smoke test passed cold boot, normal playback, rapid track changes, and storage sleep/wake without a freeze, reset, disk error, unusual delay, or other observed regression; a performance dump was captured.
...
- [ ] **P1:** find a measured DMA crossover by size/direction: every accepted request performs whole-cache maintenance on an 8 KiB cache, so small reads may be cheaper in PIO even when aligned. The current qualification build uses PIO below 4096 bytes and retains DMA at 4096 bytes and above; keep or revise that threshold only after aligned 512-byte, 4096-byte, and 1 MiB `test_disk` A/B results and normal-playback telemetry.
...
- [x] **P2:** the current 512-byte logical-sector requirement is enforced during initial `IDENTIFY` and revalidated after reset and rail wake; adapters exposing a different logical sector size are rejected unless a target explicitly provides a bounded variable-sector design. Do not accept larger logical sectors for iPod Color without complete FAT/USB qualification.
...
- [x] **P0 — implementation complete, hardware qualification pending (2026-09-02):** ARC endpoint priming no longer uses `current_tick` while IRQs are masked. The iPod uses its free-running microsecond timer; prime/flush waits are bounded at 10 ms per attempt, controller reset at 100 ms, and the PP5022-specific PHY-ready wait at one second. Flush checks for an in-flight prime becoming active and permits at most two attempts. On failure, stop interrupts/DMA and soft-reset the controller before callers may reuse buffers; PP module reset is the fallback if soft reset fails. The USB thread tears down the failed connection and permits one fresh initialization per physical insertion, without a periodic retry task. A second fault leaves USB stopped until replug. Non-PP ARC targets retain a terminal panic if even controller reset cannot stop DMA; those targets are not hardware-qualified by this iPod build.
- [x] **P0 — implementation complete, hardware qualification pending (2026-09-02):** a replacement SETUP cancels both old ARC EP0 directions and clears their completion bits. EP0 notifications carry transaction generations rather than mutable request pointers; the USB thread snapshots and handles only the newest generation after any prior handler unwinds. Replies from superseded handlers are ignored. Control-OUT payloads and thread-owned response data are separate, failed data/status submissions cancel both directions instead of waiting forever, and unexpected idle EP0 completions no longer panic. This fixes the host-retry deadlock rather than waiting for the abandoned transfer to finish.
- [x] **P0 — implementation complete (2026-09-02):** ARC shutdown cancels DMA before class disconnect callbacks free transfer buffers. USB initialization publishes core state before enabling controller interrupts, and teardown cancels the no-host timer. No extra background diagnostics are enabled. WSL build/package and code review passed for `004c09eeecM-usbfix-260902`; the package contains 414 regular files, with `rockbox.ipod` SHA-256 `b620ada69f22953de2a6f54f390ecd1dc735d57bb4adee862ee3adfb6f96cd49`.
- [ ] **USB recognition qualification for this batch:** after installation, eject, disconnect and reboot into the new firmware. Perform five normal connect/eject/disconnect cycles, including menu and playback; do not hold a button while inserting, since this intentionally toggles charge-only mode. Confirm Windows exposes the drive each time, open a folder and copy/read a small disposable file, and verify playback resumes after eject. Report success count, approximate recognition delay, any freeze/unknown-device/no-drive symptom and the iPod screen shown. Repeat the 256 MiB hash pass after reconnecting on the new firmware. Controller-stall injection, interrupted SETUP traffic, other hosts/targets and the full lifecycle matrix remain untested; a successful build is not hardware qualification.
- [x] **USB reliability package installed (2026-09-02):** backed up the previous `.rockbox` to `.work/ipod-backups/pre-usbfix-004c09eeec-20260902/.rockbox`, installed the full `004c09eeecM-usbfix-260902` package, and verified all 414 installed file hashes with zero mismatches. Existing user files outside the package were preserved. The user gave positive feedback and approved committing/pushing this batch. Exact recognition-cycle counts and timings were not supplied; the detailed qualification items above remain open.
...
- [x] **P0 — implementation complete:** after a pipelined USB write fails, drain the already-primed OUT data and remaining command payload before arming a new CBW receive; ignore unrelated IN completions while that OUT receive is pending. This prevents overwriting the ARC endpoint queue on the write-error path. Failed or vanished-medium reads finish with zero-filled payloads and a failed CSW instead of exposing stale transfer-buffer data, and no further media reads are issued after failure. READ/WRITE commands reject physical block sizes too large for a nonzero pipeline chunk. WSL build and ordinary on-device transfers passed. Targeted failure-injection qualification remains pending; no continuous diagnostics were added.
...
- [ ] **P1:** tune buffering for sustained host throughput without starving control requests, input, audio teardown, or shutdown. The 64 KiB iPod Color USB-write candidate (`aa36822b64M-260902`, PIO writes) booted with zero reported DMA/PIO errors and measured 5.062, 4.989, and 5.138 MiB/s for three 256 MiB Windows copies (5.06 MiB/s average). A fresh 24 KiB control (`aa36822b64M-usb24-260902`, same correctness fixes) measured 5.297, 5.274, and 5.525 MiB/s (5.37 MiB/s average). The larger buffer demonstrated no benefit, so the generic 24 KiB default is retained. These sequential-session measurements are not a controlled causal estimate; the historical 8.44 MiB/s result was not reproduced. Both buffer sizes use the same 128 KiB allocation; command-count reduction alone is not evidence of a throughput gain.
- [x] **Batch integrity check (2026-09-02):** all three 256 MiB copies for each buffer size passed unbuffered host readback (`robocopy /J`) and SHA-256 verification against `a6d72ac7690f53be6ae46ba88506bd97302a093f7108472bd9efc3cefda06484`. The retained 24 KiB package was built in WSL and all 414 installed files verified; installed `rockbox.ipod` SHA-256 is `d0706002674cf277a0407c9d0efb8d18440c647f3a40194523258cde2ec782ce`. The saved 24 KiB snapshot shows zero DMA timeouts/PIO recovery failures before these transfers, not a post-transfer counter sample. Normal transfer integrity passed; error-path injection and the broader release matrix remain pending.
...
- [x] **P0:** ADC staleness is explicit: the PCF path tracks last-success age plus consecutive/total failed transactions in RAM, the battery model enters stale/fault states after bounded intervals, stale data blocks disk-safe authorization, and Debug plus explicitly started Battery Benchmark runs expose the state without continuous logging. The installed A1099 reported `ADC age/fail: 0s/0 fresh` and `ADC failures total: 0` after the playback and storage-wake smoke test.
- [x] **P0:** the absolute 3200 mV raw/median battery floor remains active with `POWER_INPUT` present. Main/USB detection can clear ordinary low-battery policy only after the floor recovers; stale ADC data, a pending hard-floor debounce, or voltage at/below the floor blocks disk writes, and a persistent floor violation can still force shutdown.
