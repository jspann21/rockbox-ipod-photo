...
\config{\%V(x,y,[width],[height],[font]) \%Vf([fgcolour]) \%Vb([bgcolour]) %
    ...
    \begin{itemize}
      ...
      On colour targets, skins may instead use the theme-role names
      \config{fg}, \config{bg}, \config{lss}, \config{lse}, \config{lst},
      \config{sep}, and \config{muted}. These follow changes made in the
      \setting{Colours} menu when the skin is reloaded.
